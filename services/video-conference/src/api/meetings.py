...
from src.models import AttendeeDB
...
def attendee_joined_api(meeting_id: str, attendee_id: str, db: Session = Depends(get_db)):
    ...
    attendee = db.query(AttendeeDB).filter(
        AttendeeDB.meeting_id == meeting_id,
        AttendeeDB.id == attendee_id
    ).first()
    if attendee:
...
def get_joined_attendees_api(meeting_id: str, db: Session = Depends(get_db)):
    ...
    joined = db.query(AttendeeDB).filter(
        AttendeeDB.meeting_id == meeting_id,
        AttendeeDB.joined_at.isnot(None)
    ).all()
    return {
        "meeting_id": meeting_id,
        "joined_attendees": [a.id for a in joined],
        "joined_details": [{"attendee_id": a.id, "external_user_id": a.external_user_id, "joined_at": str(a.joined_at)} for a in joined]
    }
